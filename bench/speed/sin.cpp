...
static double bench_fast_sin() {
  double acc = 0.0;
  for (size_t i = 0; i < N; i++) {
    acc += fast_sin(inputs[i]);
  }
  sink = acc;
  return acc;
}

static double bench_std_sin() {
  double acc = 0.0;
  for (size_t i = 0; i < N; i++) {
    acc += std::sin(inputs[i]);
  }
  sink = acc;
  return acc;
}
...
int main() {
  FILE *output = std::fopen("output/sin_speed.csv", "w");
  if (!output) {
  ...
  for (int i = 0; i < REPS; i++) {
    fastSpeed.push_back(time_it(bench_fast_sin));
    commonSpeed.push_back(time_it(bench_std_sin));
  }
