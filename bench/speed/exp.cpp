...
static double bench_fast_exp() {
  double acc = 0.0;
  for (size_t i = 0; i < N; i++) {
    acc += fast_exp(inputs[i]);
  }
  sink = acc;
  return acc;
}

static double bench_std_exp() {
  double acc = 0.0;
  for (size_t i = 0; i < N; i++) {
    acc += std::exp(inputs[i]);
  }
  sink = acc;
  return acc;
}
...
int main() {
  FILE *output = std::fopen("output/exp_speed.csv", "w");
  if (!output) {
  ...
  for (int i = 0; i < REPS; i++) {
    fastSpeed.push_back(time_it(bench_fast_exp));
    commonSpeed.push_back(time_it(bench_std_exp));
  }
