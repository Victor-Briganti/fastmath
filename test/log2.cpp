...
template <typename T> bool test_log2(T &max_abs_err, T &max_rel_err) {
  max_abs_err = -1;
  max_rel_err = -1;
...
int main() {
  fprintf(stderr, "[TEST] fast_log2\n");

  float max_abs_errf, max_rel_errf;
  if (!test_log2<float>(max_abs_errf, max_rel_errf))
    return EXIT_FAILURE;

  fprintf(stderr,
          "    [PASS] fast_log2<float> max_abs_errf=%.10g max_rel_errf=%.10g\n",
          max_abs_errf, max_rel_errf);

  double max_abs_errd, max_rel_errd;
  if (!test_log2<double>(max_abs_errd, max_rel_errd))
    return EXIT_FAILURE;

  fprintf(stderr,
          "    [PASS] fast_log2<double> max_abs_err=%.10g max_rel_err=%.10g\n",
