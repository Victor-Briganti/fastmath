...
template <typename T> bool test_asin(T &max_abs_err) {
  max_abs_err = -1;
...
int main() {
  fprintf(stderr, "[TEST] fast_asin\n");

  float max_abs_errf;
  if (!test_asin<float>(max_abs_errf))
    return EXIT_FAILURE;

  fprintf(stderr, "    [PASS] fast_asin<float> max_abs_errf=%.10g\n",
          max_abs_errf);

  double max_abs_errd;
  if (!test_asin<double>(max_abs_errd))
    return EXIT_FAILURE;

  fprintf(stderr, "    [PASS] fast_asin<double> max_abs_err=%.10g\n",
          max_abs_errd);
  return EXIT_SUCCESS;
}
