...
template <typename T> bool test_acos(T &max_abs_err) {
  max_abs_err = -1;
...
int main() {
  fprintf(stderr, "[TEST] fast_acos\n");

  float max_abs_errf;
  if (!test_acos<float>(max_abs_errf))
    return EXIT_FAILURE;

  fprintf(stderr, "    [PASS] fast_acos<float> max_abs_errf=%.10g\n",
          max_abs_errf);

  double max_abs_errd;
  if (!test_acos<double>(max_abs_errd))
    return EXIT_FAILURE;

  fprintf(stderr, "    [PASS] fast_acos<double> max_abs_err=%.10g\n",
          max_abs_errd);
  return EXIT_SUCCESS;
}
